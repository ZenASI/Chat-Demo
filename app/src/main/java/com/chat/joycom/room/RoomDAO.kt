...
interface RoomDAO {
    ...
    @Query("Select * From ${Message.TABLE_NAME} Where (from_user_id = :userId And to_user_id = :selfId) Or (from_user_id = :selfId And to_user_id = :userId) Order By id Desc Limit (:nextKey - :prevKey) Offset :prevKey")
    fun pagingMessageByUserId(selfId: Long, userId: Long, prevKey: Int, nextKey: Int): List<Message>

    @Query("Select * From ${Message.TABLE_NAME} Where to_group_id = :groupId")
    fun queryMessageByGroupId(groupId: Long): Flow<List<Message>>

    @Query("Select * From ${Message.TABLE_NAME} Where to_group_id = :groupId Order By id Desc Limit (:nextKey - :prevKey) Offset :prevKey")
    fun pagingMessageByGroupId(groupId: Long, prevKey: Int, nextKey: Int): List<Message>
